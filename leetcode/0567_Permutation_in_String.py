...
class Solution:
    ...
    def checkInclusion(self, s1, s2):
        ...
        for letter in s1:
            available[letter] = 1 + available.get(letter, 0)
        current_available = available.copy()
        
        zero_counter = len(available)
        left, right = 0, 0

        while right < len(s2):
            if s2[right] in current_available:
                if current_available[s2[right]] > 0:
                    current_available[s2[right]] -= 1
                    ...
                    if current_available[s2[left]] == 0:
                        zero_counter += 1

                    current_available[s2[left]] += 1
                    left += 1
