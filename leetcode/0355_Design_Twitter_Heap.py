...
class Twitter:
    ...
    def getNewsFeed(self, userId):
        ...
        for user in possible_users_id:
            heapq.heappush(heap, (self.users_tweets[user][possible_users_id[user]], user))
        while heap and len(posts) < 10:
            posts.append(self.post_id[heap[0][0]])
            if len(self.users_tweets[heap[0][1]]) + possible_users_id[heap[0][1]] > 0:
                curr_user = heap[0][1]
                possible_users_id[heap[0][1]] -= 1
                heapq.heappop(heap)
                heapq.heappush(heap, (self.users_tweets[curr_user][possible_users_id[curr_user]], curr_user))
            else:
                heapq.heappop(heap)
